...
public class LabelService implements LabelUseCase {
    ...
    public List<Label> findByLabelType(LabelType labelType) {
        return labelPersistencePort.findByLabelType(labelType);
    }

    public boolean isLabelExistsByUserAndLabel(User user, Label label) {
        return labelPersistencePort.existsUserLabelByUserAndLabel(user, label);
    }
